...
class main:                             
    ...
    def write_json(self, data_insert):
        ...
        def overwrite(data_insert):
            with open(self.file_json, "w") as file_w:
                js.dump(data_insert, file_w, indent=4)
        ...
        if input(f"Deseja visualizar o conteúdo do {self.file_temp_json} ?\nOBS: fazer isso fará o arquivo real salvar esses dados (s/n)").lower() == "s":
            print(js.dumps(self.memory_working_temp, indent=4))
            with open(self.file_json, "w") as real_file:
                js.dump(self.memory_working_temp, real_file, indent=4)
                os.remove(self.file_temp_json)
                print("arquivo temporario deletado com sucesso")
        else:
            print("arquivo temporario ainda existe")
